...
class SFMLoss:
    ...
    def __call__(self, denoiser_net, encoder_net, img_clean, img_lr, labels, augment_pipe):
        ...
        weight = (sampled_sigma**2 + self.sigma_data**2) / (sampled_sigma * self.sigma_data) ** 2
        ...
        if len(self.sigma_min) > 1:
            sigma_t = sigma_max_per_channel.unsqueeze(0).unsqueeze(2).unsqueeze(2) * sampled_sigma
        else:
        ...
        if self.encoder_loss_type == 'l1':
            ...
            encoder_loss = torch.tensor(0.0) # This covers the case where there is no encoder_loss
